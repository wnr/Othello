...
public interface TurnRotator {
	 ...
	 */
	List<String> updateNextPlayerInTurn(Othello othello);

	/**
	 * Initiates the turn rotation. Will set the first player in turn to the given starting player. Also sets the
	 * players to be used by this object when calculating future players in turn. Should normally be the full player
	 * list in the game.
	 * 
	 * @param playerIds All the players playing the game
	 * @param startingPlayerId The ID of the player that shall be the starting player
	 */
	void initialize(List<String> playerIds, String startingPlayerId);
