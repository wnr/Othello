...
/**
 * A class encapsulating the tournament results of a player. Keeps track of the
 * number of wins, losses and draws of the player and provides methods for comparing
 * different results.
 */
public class PlayerResult implements Comparable<PlayerResult> {
    ...
    /**
     * Determines if the score of two PlayerResults are equal. Only
     * wins and draws are taken into account.
    ...
    /**
     * Compares the number of wins and draws of two PlayerResult objects. If they are both equal,
     * lexicographical ordering is used
     */
    @Override
    public int compareTo(PlayerResult pr) {
        if (wins > pr.wins) return -1;
        if (wins < pr.wins) return 1;
        if (draws > pr.draws) return -1;
        if (draws < pr.draws) return 1;
